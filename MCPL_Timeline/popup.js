...
Popup.show = function(divObject, referenceObject, position, options, modal) {
	...
	i = parseInt(referenceObject.substr(4));
	var time = x[i].getElementsByTagName("Event_Date")[0].childNodes[0].nodeValue;
	var datePieces = time.split("-");
	var year = datePieces[0]; 
	var monthNum = parseInt(datePieces[1], 10);
	var monthArray = window.monthArray;
	var month = monthArray[monthNum];
	var day = datePieces[2];
	var head = "<h3><center>" + x[i].getElementsByTagName("Brief_Description")[0].childNodes[0].nodeValue + "</center></h3>";
	var date = "<p>Date: " + month + " " + day + ", " + year + "</p>";
	var des = "";
	var madeDiv = "." + divObject;
	...
	var tags = Popup.liveTags("tag", x, i);
	var button = "<a href='#' onclick='Popup.hideAll()' style='text-decoration:none;'><img src='roundx.png' height='15px' width='15px'  style='float: right; position: relative; right: 1px; top: 5px; border:none;'></a>"; 
//	if(x[i].getElementsByTagName("Tags").length > 0)
//     	{tags = "<p>Tags: " + x[i].getElementsByTagName("Tags")[0].childNodes[0].nodeValue + "</p>";}
	var comb = button + head + date + des + link + source + tags;
	$(madeDiv).prepend(comb);
	//$(madeDiv).css('height', '200px;');
	return popup;
};

//A function 
Popup.liveTags = function(tagString, x, i){
	var parsedTags = "";
	if(x[i].getElementsByTagName("Tags").length > 0)
     	{tagString = x[i].getElementsByTagName("Tags")[0].childNodes[0].nodeValue;
		first = window.firstYear;
		last = window.endYear;
		parsedTags = tagString.replace(/([a-zA-Z]+)/g, "<a href=\"#\" onClick=\"resolveDisplaySet('Tags', '$1', '1900', '2012')\">$1</a>");
		}
	return parsedTags;
}
