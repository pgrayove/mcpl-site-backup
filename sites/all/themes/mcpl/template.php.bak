 ...
 *   entirety and paste it here, changing the prefix from theme_ to mcpl_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: mcpl_breadcrumb()
 *
 *   where mcpl is the name of your sub-theme. For example, the
 ...
 */
function mcpl_theme(&$existing, $type, $theme, $path) {
...
function mcpl_preprocess(&$vars, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function mcpl_preprocess_page(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
 ...
 */
/* -- Delete this line if you want to use this function
function mcpl_preprocess_node(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');

  // Optionally, run node-type-specific preprocess functions, like
  // mcpl_preprocess_node_page() or mcpl_preprocess_node_story().
 ...
 */
/* -- Delete this line if you want to use this function
function mcpl_preprocess_comment(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function mcpl_preprocess_block(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 *
 */
function mcpl_preprocess_ddblock_cycle_block_content(&$vars) {
  if ($vars['output_type'] == 'view_fields') {
    ...
    if ($vars['settings']['view_name'] == 'teens_highlight_items') {
      foreach ($vars['content'] as $key1 => $result) {
        // add slide_image variable 
        if (isset($result->node_data_field_pager_item_text_field_image_fid)) {
          // get image id
...
*
*/
function mcpl_preprocess_ddblock_cycle_pager_content(&$vars) {
  if (($vars['output_type'] == 'view_fields') && ($vars['pager_settings']['pager'] == 'custom-pager')){
    ...
    if ($vars['pager_settings']['view_name'] == 'teens_highlight_items') {
      foreach ($vars['content'] as $key1 => $result) {
        // add pager_item_image variable
        if (isset($result->node_data_field_pager_item_text_field_image_fid)) {
          $fid = $result->node_data_field_pager_item_text_field_image_fid;
...
function mcpl_print_terms($node, $vid = NULL, $ordered_list = TRUE) {
         ...
         if ($vocabulary->vid == $vid) {
           ...
           if ($terms) {
             ...
             foreach ($terms as $term) {
               $links[] = '<span class="term-' . $term->tid . '">' . l($term->name, taxonomy_term_path($term), array('rel' => 'tag', 'title' => strip_tags($term->description))) .'</span><br />';
             }
       ...
       foreach($vocabularies as $vocabulary) {
         if ($vocabularies) {
           ...
           if ($terms) {
             ...
             if ($ordered_list) $output .= '<li class="vocabulary-'. $vocabulary->vid . '">' . $vocabulary->name . ': ';
             $output .= '<br />'; //Remove this line to display values on the same lines as labels.
             foreach ($terms as $term) {               
               $links[] = '<span class="term-' . $term->tid . '">' . l($term->name, taxonomy_term_path($term), array('rel' => 'tag', 'title' => strip_tags($term->description))) .'</span><br />';
             }
             $output .= implode(' ', $links); //add comma between these single quotes to make multiple terms appear as a comma-delineated list.
             if ($ordered_list) $output .= '</li>';
             $output .= '</ul>';
           }
         }
...
function mcpl_array_sort_func($a,$b=NULL) {
	static $keys;
...
function mcpl_array_sort(&$array) {
	if(!$array) return $keys;
	$keys=func_get_args();
	array_shift($keys);
	mcpl_array_sort_func($keys);
	usort($array,"mcpl_array_sort_func");
}